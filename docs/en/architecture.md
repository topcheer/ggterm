...
GGTerm is a GPU-accelerated, AI-native, cross-platform terminal emulator written in Rust. It targets desktop (macOS/Linux/Windows) and mobile (iOS/Android) platforms.
...
| `ggterm-app` | 43,412 | Desktop app: winit window, event loop, tabs, splits, config, handlers |
...
| `ggterm-ai` | 2,250 | AI engine bridge (OpenAI-compatible API client) |
...
The terminal engine implements a complete VT100/VT220/xterm-compatible terminal state machine.

#### VTE Parser

Based on the Paul Williams state machine (`vte/parser.rs`). Supports:
- CSI (Control Sequence Introducer) — all standard and private modes
...
- SOS/PM/APC — consumed (no payload printed)
...
- Palette overrides (OSC 4, base16-shell compatible)
...
Implementations: `PtySession` (local PTY), `SshSession` (russh), `P2pTransport` (iroh QUIC).
...
1. **Grid → TextRuns** (`converter.rs`): Each grid row is converted to `TextRun` entries with theme color resolution. Wide chars (CJK/emoji) force run splits.

2. **TextRun → glyphon TextAreas** (`lib.rs`): Each run becomes an independent `glyphon::Buffer` positioned at `(start_col × cell_width, row × cell_height)`.

3. **Decorations** (`lib.rs`): Underlines, strikethroughs, overlines generated as vertex data in `prepare_decorations()`.

4. **SDF UI** (`ui.wgsl`): Rounded rectangles for tab bar, status bar, menus, dialogs via signed distance field shader.

5. **Multi-Pane Rendering** (`gpu.rs`):
   - One render pass for all panes
   - Each pane: `set_scissor_rect()` → `set_viewport_offset()` → `render_pane_to_pass()`
   - Final overlay pass: `render_overlays_to_pass()` (borders, tab bar, status bar, menus)

#### Font Handling

- **macOS**: Menlo (Weight::NORMAL only — Bold variant missing box-drawing glyphs)
- **Linux**: DejaVu Sans Mono
- **Windows**: Cascadia Mono
- Bold is distinguished by bright color, not weight (xterm/Alacritty standard)
- `Shaping::Advanced` enables CJK font fallback
...
Hot-reload: `config-watch` feature uses `notify` crate to watch the config file. Changes detected in `about_to_wait()` are applied immediately (theme switch, font resize, scrollback update).

### 5. Mobile FFI (`ggterm-ffi`)

C-ABI functions for Flutter integration:
...
Global session registry via `OnceLock<Mutex<HashMap<u32, MobileSession>>>`. Mutex locks use `unwrap_or_else(|e| e.into_inner())` for panic safety.

### 6. P2P Terminal Sharing (`ggterm-p2p`)

Uses iroh 1.0 (QUIC + NAT traversal) for direct mobile↔desktop terminal sharing.
...
- Ticket format: base32(postcard(EndpointAddr)) — ~130 chars, fits one QR code
- P2P direct connection success rate: 90%+ (iroh relay fallback)
- Zero operational cost (iroh public relay is free)
- Background tokio task bridges async QUIC ↔ sync buffers
...
Dual runtime: Lua (via `mlua`) and WASM. Configured via:
...
Example plugin: `examples/plugins/hello.lua`
...
OSC 133 marks for command detection:
- `OSC 133 ; A ST` — prompt start
- `OSC 133 ; B ST` — command start
- `OSC 133 ; C ST` — output start
- `OSC 133 ; D ; <exit_code> ST` — command exit

Supported shells: bash, zsh, fish (auto-injected via `shell/` scripts).
...
| `plugin-lua` | ggterm-app | Lua plugin runtime (implies `plugin`) |
