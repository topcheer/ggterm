...
impl Parser {
    ...
    fn osc_string<P: Perform>(&mut self, byte: u8, perform: &mut P) {
        match byte {
            ...
            c if c >= 0x20 => {
                // Buffer at 64KB cap — silently consume overflow bytes
                // until the terminator (BEL or ST) arrives, preventing them
                // from being processed as terminal output. No per-byte
                // logging to avoid log flooding on large malformed sequences.
            }
            _ => {}
        }
    }
    ...
    fn dcs_string<P: Perform>(&mut self, byte: u8, perform: &mut P) {
        match byte {
            ...
            c if c >= 0x20 => {
                // Buffer at 1MB cap — silently consume overflow bytes
                // until the terminator arrives, preventing them from
                // being processed as terminal output.
            }
            _ => {}
        }
    }
