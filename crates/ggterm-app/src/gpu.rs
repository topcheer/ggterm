...
pub fn cursor_state(app: &crate::App) -> ggterm_render::CursorState {
    ...
    let cursor_color = app.terminal().dynamic_cursor().map(|c| match *c {
        ...
        ggterm_core::Color::Indexed(idx) => {
            // Check user palette overrides (OSC 4) first, then fall back to standard 256-color.
            if let Some(&(r, g, b)) = app.terminal().palette_overrides().get(&idx) {
                (r, g, b)
            } else {
                ggterm_render_wgpu::colors::indexed_to_rgb(idx)
            }
        }
