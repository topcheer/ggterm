...
/// Nushell shell integration script.
pub const NU_INTEGRATION: &str = include_str!("../../../shell/nu.nu");
...
impl ShellKind {
    ...
    pub fn integration_script(self) -> Option<&'static str> {
        match self {
            ...
            Self::Nushell => Some(NU_INTEGRATION),
            Self::Unknown => None,
        }
    }
}
...
impl ShellIntegrationConfig {
    ...
    pub fn spawn_args(&self) -> (String, Vec<String>) {
        ...
        match self.kind {
            ...
            ShellKind::Nushell => {
                // Write a merged config.nu that sources both the user's
                // existing config (if any) and our integration script.
                let nu_config = self.temp_dir.join("config.nu");
                let home = std::env::var("HOME").unwrap_or_default();
                let user_config = format!("{home}/.config/nushell/config.nu");
                let _ = fs::write(
                    &nu_config,
                    format!(
                        "# Auto-generated by GGTerm\n\
                         try {{ source '{user_config}' }}\n\
                         try {{ source '{script_str}' }}\n"
                    ),
                );
                let config_str = nu_config.to_string_lossy().into_owned();
                (self.shell_path.clone(), vec!["--config".into(), config_str])
            }
...
fn script_filename(kind: ShellKind) -> &'static str {
    match kind {
        ...
        ShellKind::Nushell => "ggterm_nu.nu",
...
pub fn install_scripts_manually() -> std::io::Result<Vec<ManualInstallResult>> {
    ...
    for (kind, script) in [
        ...
        (ShellKind::Nushell, NU_INTEGRATION),
    ] {
...
mod tests {
    ...
    #[test]
    fn t_detect_nushell() {
        assert_eq!(ShellKind::from_path("/usr/bin/nu"), ShellKind::Nushell);
        assert_eq!(
            ShellKind::from_path("/opt/homebrew/bin/nu"),
            ShellKind::Nushell
        );
        assert_eq!(ShellKind::from_path("nushell"), ShellKind::Nushell);
    }
    ...
    fn t_detect_unknown() {
        ...
        assert_eq!(ShellKind::from_path("/bin/nuke"), ShellKind::Unknown);
    }
    ...
    fn t_integration_scripts_exist() {
        ...
        assert!(!NU_INTEGRATION.is_empty());
        assert!(NU_INTEGRATION.contains("133"));
    }
    ...
    #[test]
    fn t_prepare_nushell() {
        let config = ShellIntegrationConfig::prepare("/usr/bin/nu");
        assert!(config.prepared);
        assert_eq!(config.kind, ShellKind::Nushell);
        let (_, args) = config.spawn_args();
    ...
    fn t_script_filename() {
        ...
        assert_eq!(script_filename(ShellKind::Nushell), "ggterm_nu.nu");
    }
    ...
    fn t_manual_install_writes_scripts() {
        ...
        assert_eq!(results.len(), 4, "should write bash+zsh+fish+nu scripts");
