...
impl ApplicationHandler for DesktopApp {
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                // Selection count (live feedback while selecting).
                ...
                let sel_lines = self
                    .selection
                    .normalized()
                    .map(|((_, sy), (_, ey))| (ey - sy + 1) as usize)
                    .unwrap_or(0);
                self.status_bar.selection_count = sel_chars;
                self.status_bar.selection_lines = sel_lines;
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Uses disjoint field borrow: self.sessions (immutable for grid)
        // and self.search (mutable) — avoids cloning the entire grid.
        if active_had_data && self.search.visible {
            let grid = self.sessions[self.active].app().grid();
            self.search.refresh(grid);
        }
